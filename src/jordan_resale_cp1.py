...
feature_cols = [c for c in all_cols if c not in target_cols + leak_cols + datetime_cols + extra_drop + id_cols]
...
vc = df['is_profitable'].value_counts()
axes[0, 0].bar(
    ['Убыточная (0)', 'Прибыльная (1)'], vc.values, color=['#e74c3c', '#2ecc71'], edgecolor='white', linewidth=1.5
)
axes[0, 0].set_title('Баланс классов')
axes[0, 0].set_ylabel('Количество сделок')
for i, v in enumerate(vc.values):
    axes[0, 0].text(i, v + len(df) * 0.005, f'{v:,}\n({v / len(df) * 100:.1f}%)', ha='center', fontweight='bold')
...
print(
    f'Баланс классов: {profit_share:.1f}% сделок прибыльны - классы '
    + ('умеренно несбалансированы' if 40 < profit_share < 60 else 'несбалансированы')
    + '. Использован class_weight="balanced".'
)
median_profit = df['profit'].median()
print(
    f'Медианная прибыль: ${median_profit:.0f}. Распределение прибыли смещено - большинство сделок дают небольшую маржу.'
)
print(
    'График Retail vs Resale показывает, что дорогие кроссовки (>$200) '
    'чаще уходят с прибылью - видна чёткая кластеризация зелёных точек выше диагонали.'
)
print('Розничные цены сконцентрированы в диапазоне $100–$250, что типично для массовых релизов Jordan.')

fig, ax = plt.subplots(figsize=(9, 7))
sns.heatmap(X.corr(), annot=True, fmt='.2f', cmap='coolwarm', center=0, linewidths=0.5, ax=ax)
...
print(
    'Корреляционная матрица: большинство признаков слабо коррелируют между собой. '
...
rf = RandomForestClassifier(
    n_estimators=100, max_depth=8, random_state=RANDOM_STATE, class_weight='balanced', n_jobs=-1
)
...
print('\n-- 8.3 Gradient Boosting --')
gbm = GradientBoostingClassifier(n_estimators=100, max_depth=4, learning_rate=0.1, random_state=RANDOM_STATE)
...
ada = AdaBoostClassifier(
    estimator=DecisionTreeClassifier(max_depth=2), n_estimators=100, learning_rate=0.5, random_state=RANDOM_STATE
)
...
# --- 8.5 Extra Trees ---
print('\n-- 8.5 Extra Trees --')
et = ExtraTreesClassifier(n_estimators=100, max_depth=8, random_state=RANDOM_STATE, class_weight='balanced', n_jobs=-1)
...
voting = VotingClassifier(estimators=[('rf', rf), ('gbm', gbm), ('et', et)], voting='soft', n_jobs=-1)
...
# ===
# 8.7 ПОДБОР ГИПЕРПАРАМЕТРОВ (GridSearchCV для нескольких моделей)
# ===
print('\n-- 8.7 GridSearchCV - подбор гиперпараметров --')

# --- 8.7.1 Random Forest GridSearch ---
print('\n  [RF] Гипотеза: увеличение числа деревьев и глубины улучшит качество RF')
rf_param_grid = {
...
rf_gs_pred = rf_grid.best_estimator_.predict(X_test)
rf_gs_prob = rf_grid.best_estimator_.predict_proba(X_test)[:, 1]
rf_gs_f1 = f1_score(y_test, rf_gs_pred, average='macro')
rf_gs_auc = roc_auc_score(y_test, rf_gs_prob)
print(f'  Лучшие параметры: {rf_grid.best_params_}')
print(f'  F1-macro (CV): {rf_grid.best_score_:.4f} | F1-macro (test): {rf_gs_f1:.4f} | ROC-AUC: {rf_gs_auc:.4f}')
results['RF (GridSearch)'] = {'f1': rf_gs_f1, 'auc': rf_gs_auc, 'pred': rf_gs_pred, 'params': rf_grid.best_params_}

# --- 8.7.2 Gradient Boosting GridSearch ---
print('\n  [GBM] Гипотеза: подбор learning_rate и глубины улучшит GBM')
gbm_param_grid = {
    'n_estimators': [100, 200, 300],
    'max_depth': [3, 4, 6],
...
gbm_gs_pred = gbm_grid.best_estimator_.predict(X_test)
gbm_gs_prob = gbm_grid.best_estimator_.predict_proba(X_test)[:, 1]
gbm_gs_f1 = f1_score(y_test, gbm_gs_pred, average='macro')
gbm_gs_auc = roc_auc_score(y_test, gbm_gs_prob)
print(f'  Лучшие параметры: {gbm_grid.best_params_}')
print(f'  F1-macro (CV): {gbm_grid.best_score_:.4f} | F1-macro (test): {gbm_gs_f1:.4f} | ROC-AUC: {gbm_gs_auc:.4f}')
results['GBM (GridSearch)'] = {
    'f1': gbm_gs_f1,
    'auc': gbm_gs_auc,
    'pred': gbm_gs_pred,
...
# --- 8.7.3 Extra Trees GridSearch ---
print('\n  [ET] Гипотеза: оптимизация числа деревьев и min_samples_leaf для ET')
et_param_grid = {
    'n_estimators': [100, 200, 300],
    'max_depth': [6, 8, 12, None],
    'min_samples_leaf': [1, 3, 5],
...
et_gs_pred = et_grid.best_estimator_.predict(X_test)
et_gs_prob = et_grid.best_estimator_.predict_proba(X_test)[:, 1]
et_gs_f1 = f1_score(y_test, et_gs_pred, average='macro')
et_gs_auc = roc_auc_score(y_test, et_gs_prob)
print(f'  Лучшие параметры: {et_grid.best_params_}')
print(f'  F1-macro (CV): {et_grid.best_score_:.4f} | F1-macro (test): {et_gs_f1:.4f} | ROC-AUC: {et_gs_auc:.4f}')
results['ET (GridSearch)'] = {'f1': et_gs_f1, 'auc': et_gs_auc, 'pred': et_gs_pred, 'params': et_grid.best_params_}

# --- 8.7.4 AdaBoost GridSearch ---
print('\n  [AdaBoost] Гипотеза: увеличение глубины базового дерева и подбор lr улучшит AdaBoost')
ada_param_grid = {
...
ada_gs_pred = ada_grid.best_estimator_.predict(X_test)
ada_gs_prob = ada_grid.best_estimator_.predict_proba(X_test)[:, 1]
ada_gs_f1 = f1_score(y_test, ada_gs_pred, average='macro')
ada_gs_auc = roc_auc_score(y_test, ada_gs_prob)
print(f'  Лучшие параметры: {ada_grid.best_params_}')
print(f'  F1-macro (CV): {ada_grid.best_score_:.4f} | F1-macro (test): {ada_gs_f1:.4f} | ROC-AUC: {ada_gs_auc:.4f}')
results['AdaBoost (GridSearch)'] = {
    'f1': ada_gs_f1,
    'auc': ada_gs_auc,
    'pred': ada_gs_pred,
...
best_rf = rf_grid.best_estimator_  # для feature importance далее
...
hypotheses = {
    ...
    'GBM (GridSearch)': 'Подбор гиперпараметров улучшает GBM',
    'ET (GridSearch)': 'Подбор гиперпараметров улучшает ET',
    'AdaBoost (GridSearch)': 'Подбор гиперпараметров улучшает AdaBoost',
}
...
ConfusionMatrixDisplay.from_predictions(
    y_test, lr_pred, ax=axes[0], display_labels=['Убыточная', 'Прибыльная'], colorbar=False, cmap='Blues'
)
axes[0].set_title('Logistic Regression (Baseline)')

ConfusionMatrixDisplay.from_predictions(
    y_test, best_pred, ax=axes[1], display_labels=['Убыточная', 'Прибыльная'], colorbar=False, cmap='Greens'
)
